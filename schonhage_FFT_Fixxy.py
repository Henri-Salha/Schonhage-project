...
def mult_fft (a,b, add_precision=0):
    ...
    a2 = myfft(ac,n,add_precision)# array of Coffy
    b2 = myfft(bc,n,add_precision)  # array of Coffy
    c2 = [x*y for x,y in zip(a2,b2)] # array of Coffy
    c1 = myifft (c2,n,add_precision) # array of Coffy
    m = extract(c1) # Coffy
...
def comp (a,n,precision): #à faire
    x = fx.Coffy.fromvalue(0,0,precision)
    for i in range(pow(2,n)):
        y = a[i].pow2(n*(i))
        # z = a[i].real
        # if abs(a[i].real.mantissa)<pow(2,n): y = fx.Coffy.fromvalue(0,0,precision)
        x = x + y
        #print ("a[",i,"]:",a[i].real.mantissa,a[i].imag.mantissa,a[i].real.exp,"result=",np.round(x.toComplex().real,10))
    return x
...
def myfft (a, n,add_precision):
    'tf de Fourier discrète'
    precision = a[0].real.exp-add_precision
    a0 = [x.more_precision(add_precision) for x in a]
    base = pow(2,n)
    ...
    for i in range(base):
        ...
        for k in range(base):
            ...
            a1[i] = a1[i] + a0[k]*w[k1]
            ra = (a0[k].real.mantissa, a0[k].imag.mantissa)
            rw = (w[k1].real.mantissa, w[k1].imag.mantissa)
            r1 = (a1[i].real.mantissa, a1[i].imag.mantissa)
            if (ra[0] | ra[1]):
                #print(f"a[{k}]={ra}* w[{k1}]={rw} = {r1}+")
                #print(i,k,ra[0],ra[1],rw[0],rw[1],r1[0],r1[1])
                continue
        #print(f"= fft[{i}]")
    a2 = [x.drop_precision(add_precision) for x in a1]
    return a2

def myifft (a, n, add_precision):
    ...
    for i in range(base):
        ...
        for k in range(base):
            k1 = (i*k) % base # prend le modulo 
            a1[i] = a1[i] + (a0[k]*w[k1]) #div par 2^n
            ra = (a0[k].real.mantissa, a0[k].imag.mantissa)
            rw = (w[k1].real.mantissa, w[k1].imag.mantissa)
            r1 = (a1[i].real.mantissa, a1[i].imag.mantissa)
            if (ra[0] | ra[1]):
...
#ss = diff_mult(t1, t2)
#b = mult_fft(t1,t2)
#c =[[w.toComplex() for w in b[n]] for n in range(6)]+[b[6],b[7]]
